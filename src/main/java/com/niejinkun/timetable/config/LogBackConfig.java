...
/*	@Bean(name = "notice ", initMethod = "start", destroyMethod = "stop")
	public static RollingFileAppender rollingAppend(LoggerContext ctx, PatternLayoutEncoder encoder) {
		RollingFileAppender appender = new RollingFileAppender();
		appender.setAppend(true);
		appender.setName("notice");
		appender.setFile("../logs/notice.log");
		appender.setRollingPolicy((RollingPolicy) timePolicy());
	...
	@Bean
	public static TimeBasedRollingPolicy timePolicy(){
		TimeBasedRollingPolicy timePolicy = new TimeBasedRollingPolicy();
		timePolicy.setMaxHistory(7);
		timePolicy.setFileNamePattern("../logs/notice.%d{yyyy-MM-dd-HH-mm}.log");
